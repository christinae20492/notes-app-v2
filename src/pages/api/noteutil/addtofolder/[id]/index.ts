...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  console.log(`API: User ${userId} is moving note ${noteId} to folder ${folderId}.`);
  ...
  try {
    ...
    return res.status(200).json({ message: 'Note moved to folder successfully!', note: noteToMove });
  } catch (error: any) {
    if (error.code === 'P2025') {
